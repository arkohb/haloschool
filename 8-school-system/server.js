...
/* seed super admin */
(function seedSuper() {
  if (db.prepare("SELECT id FROM users WHERE role='super' LIMIT 1").get()) return;
  const login = (process.env.SUPER_ADMIN_EMAIL || "super@haloschool.app").toLowerCase();
  ...
  db.prepare(`INSERT INTO users(school_id,role,name,login_id,email,pass_hash,salt,status)
    ...
    .run(login, login, hashPassword(pass, salt), salt);
  console.log("==================================================");
  console.log(" SUPER ADMIN  login: " + login + "   password: " + pass);
